...
public class DeviceDatabase {
  ...
  public DeviceDatabase(String groupname, String filename, DeviceManagerImpl devicemanager) {
    ...
    try {
      ...
      if (event.hasVariables()) {
        ...
        for (VariableUpdate v : event.getVariables()) {
          ...
          devicemanager.fireDeviceChanged(v.getDevice(), DeviceManagerImpl.DeviceChangedType.Variable, v.getVariable());
        }
      }
    ...
    for (LocationUpdate update : updates) {
      switch (update.getType()) {
        case Global:
          ...
          break;
        case Local:
          LocationLocal llocation = new LocationLocal(update.getDevice(), update.getPoint(), /*TODO: fix sender*/ "sender1", (update.isStaticLocation() ? new Date(0) : new Date()), update.getRoom());
